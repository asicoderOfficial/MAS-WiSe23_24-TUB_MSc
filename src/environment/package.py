...
class Package:
    ...
    def __init__(self, id: str, position: Position, destination: Position, max_iterations_to_deliver: int, intermediate_point: Position =None, destination_pp = None) -> None:
        ...
        self.picked = False
        self.intermediate_point_pos = intermediate_point
    ...
    def __str__(self) -> str:
        line = f"Package {self.id} at {self.pos} to {self.destination}"
        if self.intermediate_point_pos != None:
            line += f" through {self.intermediate_point_pos}"
        return line