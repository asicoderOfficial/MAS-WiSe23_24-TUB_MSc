...
class Agent(MesaAgent):
    ...
    def step(self, grid) -> None:
        ...
        if self.algorithm_name == 'dijkstra':
            chosen_new_position = self.algorithm.get_next_position(self, self.pos, self.packages[0].destination, grid.height, grid.width, convert_grid_to_matrix(grid), count)
        elif self.algorithm_name == 'pheromones':
            chosen_new_position = self.algorithm.get_next_position(self.pos, self.origin, None, self.packages[0].destination, None, perception, grid, True, True)
        else:
            raise Exception(f"Unknown algorithm: {self.algorithm_name}")
